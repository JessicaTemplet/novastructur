    ...
    <div className="flex h-full overflow-hidden">
      <div className="flex-1 overflow-y-auto">
        <div className="flex items-center justify-between border-b border-neutral-200 bg-white px-4 py-2.5">
          <div className="flex items-center gap-2 text-sm text-neutral-500">
            <Link href="/" className="flex items-center gap-1 hover:text-neutral-800">
        ...
        <div className="mx-auto max-w-2xl px-6 py-6">
          ...
          <div className="relative mt-2">
            ...
            <button
              ...
              className="absolute right-0 top-0 flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-indigo-600 hover:bg-indigo-50 disabled:opacity-40"
              title="Draft with your configured AI provider (Settings → AI)"
            >
              <Sparkles className="h-3 w-3" /> {draft.isPending ? "Drafting…" : "AI draft"}
            </button>
          </div>
          {draft.isError && <p className="text-xs text-red-600">{draft.error.message}</p>}
      ...
      <aside className="w-64 shrink-0 border-l border-neutral-200 bg-white p-4">
        <div className="space-y-4 text-sm">
          <div>
            <div className="mb-1 text-xs font-medium text-neutral-400">Status</div>
            <StatusPicker issueId={issue.id} current={issue.state} options={statusOptions} />
            <span className="ml-1.5 text-sm text-neutral-700">{issue.state.name}</span>
          </div>
          <div>
            <div className="mb-1 text-xs font-medium text-neutral-400">Priority</div>
            <PriorityPicker issueId={issue.id} current={issue.priority} />
            <span className="ml-1.5 text-sm text-neutral-700">{PRIORITY_META[issue.priority].label}</span>
          </div>
          <div>
            <div className="mb-1 text-xs font-medium text-neutral-400">Assignee</div>
            <div className="flex items-center gap-1.5">
              <AssigneePicker issueId={issue.id} current={issue.assignee} />
              <span className="text-sm text-neutral-700">{issue.assignee?.name ?? "Unassigned"}</span>
            </div>
          </div>
          <div>
            <div className="mb-1 text-xs font-medium text-neutral-400">Cycle</div>
            <CyclePicker issueId={issue.id} current={issue.cycle} cycles={cycles} />
          </div>
          <div>
            <div className="mb-1 text-xs font-medium text-neutral-400">Labels</div>
          ...
          <div>
            <div className="mb-1 text-xs font-medium text-neutral-400">Estimate</div>
            <input
              type="number"
              value={estimate}
              ...
              placeholder="—"
              className="w-full rounded-md border border-neutral-200 px-2 py-1 text-sm outline-none focus:border-indigo-400"
            />
          </div>
          <div>
            <div className="mb-1 text-xs font-medium text-neutral-400">Due date</div>
            <input
              type="date"
              value={dueDate}
              onChange={(e) => {
